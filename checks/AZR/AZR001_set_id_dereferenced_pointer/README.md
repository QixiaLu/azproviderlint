...
The Azure API returns Resource IDs inconsistently (casing and segment order can differ between services and operations), so Terraform manages its own Resource IDs. New resources should build the ID with a generated Resource ID Formatter and call `d.SetId(id.ID())`.

A Resource ID Formatter, Parser and Validator can be generated by adding a `resourceids.go` file to the service package containing:
