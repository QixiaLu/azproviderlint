...
The AZT001 analyzer reports resource and data source acceptance test files
(`*resource*_test.go`, `*data_source*_test.go`) that use the service package instead of an
external `_test` package.

Acceptance tests must live in a `_test` package to prevent a circular dependency between
