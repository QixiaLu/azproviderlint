...
The consuming statement must be a plain single assignment (`y = x`) or a single-value `return x`. Call arguments are deliberately out of scope, since naming an argument is usually intentional documentation. Assignments whose left-hand side contains a function call are skipped, because inlining would move the temporary's initializer after the left-hand side's operands in evaluation order. Multi-value declarations, non-adjacent consumers, blank-identifier discards and temporaries captured by closures are all ignored.
