...
The AZC001 analyzer reports Azure SDK (track1 & kermit) clients being created with
`NewFoosClient(o.SubscriptionId)`.

Clients must be created with `NewFoosClientWithBaseURI(...)` so the resource manager
endpoint is explicitly specified - without it the client silently defaults to Azure Public,
breaking sovereign and non-public clouds (Azure China, US Government, etc.).
