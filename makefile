...
##@ Build
build: ## Compile azproviderlint with version info from git
	@echo "==> building..."
	go build -ldflags "-X github.com/katbyte/azproviderlint/version.GitCommit=${GIT_COMMIT} -X github.com/katbyte/azproviderlint/version.Version=${GIT_VERSION}"

install: ## Install azproviderlint into GOPATH/bin with version info from git
	@echo "==> installing..."
